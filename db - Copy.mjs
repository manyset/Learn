                ...
                if (op === 'exec') {
                    ...
                    try {
                        if (params && params.length > 0) {
                            ...
                            // Get column names by inspecting first row
                            const columnNames = [];
                            if (stmt.step()) {
                                // Get column count from the row's array length
                                const firstRow = stmt.get();
                                log(\`First row array length: \${firstRow.length}\`);
                                
                                // Generate generic column names if we can't get real ones
                                for (let i = 0; i < firstRow.length; i++) {
                                    columnNames.push(\`col\${i}\`); // Fallback generic names
                                }
                                
                                // Create first row object
                                const firstRowObj = {};
                                for (let i = 0; i < firstRow.length; i++) {
                                    firstRowObj[columnNames[i]] = firstRow[i];
                                }
                                rows.push(firstRowObj);
                            ...
                            self.db.exec({
                                ...
                                callback: (row) => {
                                    if (first) {
                                        // Get column names from the row object keys
                                        for (let col in row) {
