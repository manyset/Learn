// db-robust.mjs - Professional SQLite OPFS API
...
class SQLiteDB {
    ...
    // Initialize database - returns { success, status (created/opened) }
    async init() {
    ...
    async execThrow(sql, params = []) {
    const result = await this.exec(sql, params);
    
    if (!result.success) {
        // Convert the error object to a thrown DatabaseError
        throw new DatabaseError(
            result.code || DB_ERROR.OPERATION_FAILED,
    ...
    // Execute SQL (INSERT, UPDATE, DELETE, CREATE) - returns { success, changes, lastInsertRowid }
// Replace this entire exec method

async exec(sql, params = []) {
    if (!this.ready) {
        throw new DatabaseError(DB_ERROR.NOT_INITIALIZED, 'Database not initialized');
    }
    
    try {
        const result = await this._post('exec', sql, params);
        if (result?.error) {
            throw this._parseSQLiteError(result.error);
        }
        return { 
            success: true, 
            changes: result?.c || 0,
    ...
    // Query data - returns array of objects with column names as keys
    ...
    // Get table data formatted for grid display
...
_logError(err) {
    ...
    if (this.errorLog.length > 50) {  // Keep last 50 only
        this.errorLog.shift();
    }
    console.error('❌ DB Error:', err.userMessage || err.message);
}
