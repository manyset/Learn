// db.mjs - SQLite- WASM, OPFS, MIME, Worker based API
...
class SQLiteDB {
    ...
    // Execute SQL (INSERT, UPDATE, DELETE, CREATE) - throws on error
...
async transaction() {
    const result = await this.exec('BEGIN TRANSACTION;');
    return result;
}

async commit() {
    const result = await this.exec('COMMIT;');
    return result;
}

async rollback() {
    const result = await this.exec('ROLLBACK;');
    return result;
}
